...
class Scraper
{
    ...
    {
        $defaults = [
            'logPath' => sys_get_temp_dir().'/scrapereus/log',
            'cachePath' => sys_get_temp_dir().'/scrapereus/cache',
        ];
    ...
    public function getUserAgent()
    {
        switch ($this->userAgent) {
            ...
            default:
                return $this->getUseragentRandom();
                break;
        }
    }
    ...
    public function checkProxy($proxy):boolean
    {
        $proxy=  explode(':', $proxy);
        var_dump($proxy);
        $host = $proxy[0]; 
